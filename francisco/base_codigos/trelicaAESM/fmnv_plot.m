...
for i=1:size(elems,2)
    plot(nos(1,elems(:,i)),nos(2,elems(:,i)),'b','LineWidth',2);
end

for i=1:size(nos,2)
    plot(nos(1,i),nos(2,i),'bo','LineWidth',2,'MarkerSize',6,'MarkerFaceColor',[1,1,1]);
end
...
for i=1:size(elems,2)
    plot(nos(1,elems(:,i))+f*phi0(2*(elems(:,i)-1)+1)', ...
         nos(2,elems(:,i))+f*phi0(2*(elems(:,i)-1)+2)','r','LineWidth',2);
end

for i=1:size(nos,2)
    plot(nos(1,i)+f*phi0(2*(i-1)+1),nos(2,i)+f*phi0(2*(i-1)+2),'ro','LineWidth',2,'MarkerSize',6,'MarkerFaceColor',[1,1,1]);
end